...
def ShowUsers(request):
    if request.user.is_super_admin:
        ...
        for i in get_all_main_client:
            ...
            for j in users:
                temp.append(j)
                second_user = User.objects.filter(linked_employee = j.id)
                for k in second_user:
                    temp.append(k)
            obj = {
                "main_client":i,
                "users":temp
...
def AddMainClient(request):
    if request.user.is_super_admin:
        if request.method == 'POST':
            ...
            if client_first_name and client_last_name and client_email_id and client_user_name and client_password and client_user_creation and client_files_upload:
                print(client_first_name , client_last_name , client_email_id , client_user_name , client_password , client_user_creation , client_files_upload)
